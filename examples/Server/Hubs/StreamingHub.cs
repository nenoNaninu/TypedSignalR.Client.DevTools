...
public sealed class StreamingHub : Hub<IStreamingHubReceiver>, IStreamingHub
{
    ...
    public async IAsyncEnumerable<StreamingHubMessage> Counter(Person publisher, int init, int step, int count)
    {
        int value = init;
        ...
        for (int i = 0; i < count; i++)
        {
            this.Context.ConnectionAborted.ThrowIfCancellationRequested();
            await Task.Delay(TimeSpan.FromMilliseconds(200));
            yield return new StreamingHubMessage(publisher, value);
            value += step;
        }
    }

    public async IAsyncEnumerable<StreamingHubMessage> CancelableCounter(Person publisher, int init, int step, int count, [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        int value = init;
    ...
    public Task<IAsyncEnumerable<StreamingHubMessage>> TaskCancelableCounter(Person publisher, int init, int step, int count, CancellationToken cancellationToken)
    {
    ...
    public Task<ChannelReader<StreamingHubMessage>> CounterChannel(Person publisher, int init, int step, int count)
    {
        var channel = Channel.CreateUnbounded<StreamingHubMessage>(new UnboundedChannelOptions { SingleReader = false, SingleWriter = true });

        _ = WriteStreamingHubMessageToChannelAsync(channel.Writer, publisher, init, step, count, this.Context.ConnectionAborted);

        return Task.FromResult(channel.Reader);
    }

    public Task<ChannelReader<StreamingHubMessage>> CancelableCounterChannel(Person publisher, int init, int step, int count, CancellationToken cancellationToken)
    {
        var channel = Channel.CreateUnbounded<StreamingHubMessage>(new UnboundedChannelOptions { SingleReader = false, SingleWriter = true });
    ...
    public Task<ChannelReader<StreamingHubMessage>> TaskCancelableCounterChannel(Person publisher, int init, int step, int count, CancellationToken cancellationToken)
    {
        var channel = Channel.CreateUnbounded<StreamingHubMessage>(new UnboundedChannelOptions { SingleReader = false, SingleWriter = true });

        _ = WriteStreamingHubMessageToChannelAsync(channel.Writer, publisher, init, step, count, cancellationToken);
    ...
    public async Task UploadStream(Person publisher, IAsyncEnumerable<Person> stream)
    {
        ...
        _logger.Log(LogLevel.Information, "UploadStream: Completed");
    }
    ...
    public async Task UploadStreamAsChannel(Person publisher, ChannelReader<Person> stream)
    {
        ...
        _logger.Log(LogLevel.Information, "UploadStreamAsChannel: Completed");
    }
    ...
    private static async Task WriteStreamingHubMessageToChannelAsync(
        ChannelWriter<StreamingHubMessage> writer,
        Person publisher,
        int init,
        int step,
        int count,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                await Task.Delay(TimeSpan.FromMilliseconds(200), cancellationToken);
                await writer.WriteAsync(new StreamingHubMessage(publisher, value), cancellationToken);
